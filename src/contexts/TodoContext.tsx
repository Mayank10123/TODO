...
export const TodoProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    if (previousUserIdRef.current && previousUserIdRef.current !== userId) {
      // User changed - clear previous user's pomodoro state from localStorage
      const prevKey = userId ? `tabbie_pomodoro_state_${previousUserIdRef.current}` : 'tabbie_pomodoro_state';
      localStorage.removeItem(prevKey);
      console.log(`🔐 Cleared previous user's session data on logout/switch`);
    }
    previousUserIdRef.current = userId;
  }, [userId]);

  // Load user data when userId changes (login/logout/switch accounts)
  useEffect(() => {
    setUserData(loadUserData(userId));
    setPomodoroTimer(prev => {
      const savedState = loadPomodoroState(userId);
      if (savedState) {
  ...
  }, [userData, currentTaskId, userId]);

  // Listen for storage events to sync state across tabs
  useEffect(() => {
    const handleStorageChange = (e: StorageEvent) => {
      const pomodoroStateKey = userId ? `tabbie_pomodoro_state_${userId}` : 'tabbie_pomodoro_state';
      if (e.key === pomodoroStateKey && e.newValue) {
        try {
  ...
  useEffect(() => {
    ...
    if (pomodoroTimer.currentSession || pomodoroTimer.justCompleted) {
      savePomodoroState(pomodoroState, userId);
    } else {
      clearPomodoroState(userId);
    }
  }, [pomodoroTimer, currentTaskId, userId]);
  ...
  useEffect(() => {
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === 'pomodoro_state') {
        const newState = loadPomodoroState(userId);
        if (!newState) {
