...
const STORAGE_KEY_PREFIX = 'tabbie_user_data';
const POMODORO_STATE_KEY_PREFIX = 'tabbie_pomodoro_state';
...
// Helper function to get user-specific pomodoro state key
const getPomodoroStateKey = (userId?: string): string => {
  if (userId) {
    return `${POMODORO_STATE_KEY_PREFIX}_${userId}`;
  }
  // Fallback for non-authenticated contexts
  return POMODORO_STATE_KEY_PREFIX;
};
...
export const loadPomodoroState = (userId?: string): PomodoroState | null => {
  try {
    const storedState = localStorage.getItem(getPomodoroStateKey(userId));
    if (storedState) {
...
export const savePomodoroState = (state: PomodoroState, userId?: string): void => {
  try {
    localStorage.setItem(getPomodoroStateKey(userId), JSON.stringify(state));
  } catch (error) {
...
export const clearPomodoroState = (userId?: string): void => {
  try {
    localStorage.removeItem(getPomodoroStateKey(userId));
  } catch (error) {
...
export const saveUserData = (userData: UserData, userId?: string): void => {
  try {
    localStorage.setItem(getStorageKey(userId), JSON.stringify(userData));
  } catch (error) {
